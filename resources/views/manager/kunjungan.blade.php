        ...
        <form action="{{ route('manager.kunjungan') }}" method="GET" style="display: flex; gap: 12px; align-items: flex-end; flex-wrap: wrap;">
            ...
            <div style="display: flex; gap: 8px;">
                ...
                @if(request()->hasAny(['keyword', 'start_date', 'end_date']) || request('vip_status', 'all') !== 'all')
                    <a href="{{ route('manager.kunjungan') }}" style="background: #f1f5f9; color: #475569; text-decoration: none; padding: 10px 16px; border-radius: 10px; font-size: 13px; font-weight: 700; display: inline-flex; align-items: center; height: 41px; box-sizing: border-box;">
                        Reset
                    </a>
                @endif
            </div>
