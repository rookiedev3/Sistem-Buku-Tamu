...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
    <div>
        <h1 style="font-size: 20px; font-weight: 800; color: #172033; margin: 0 0 4px 0;">Tambah Visit Purpose Baru</h1>
        <p style="font-size: 13px; color: #778195; margin: 0;">Lengkapi formulir di bawah ini untuk menambahkan tujuan kunjungan.</p>
    </div>
    
     <a href="{{ route('visit-purposes.index') }}" style="background: #e5484d; color: #ffffff; padding: 10px 16px; border-radius: 12px; font-size: 13px; font-weight: 700; text-decoration: none; display: flex; align-items: center; gap: 6px; border: 1px solid #e8edf5; transition: all 0.2s; white-space: nowrap;">
        &larr; Kembali
    </a>
</div>
...
<div style="background: #ffffff; border: 1px solid #e8edf5; border-radius: 20px; box-shadow: 0 18px 50px rgba(31,53,97,.12); padding: 24px; width: 100%; max-width: 680px; box-sizing: border-box; margin-bottom: 32px;">
    ...
    <form action="{{ route('visit-purposes.store') }}" method="POST">
        @csrf
